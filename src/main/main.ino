...
/*
Due to the fact that the MCU running the display is slower than the one we are programming, we need to
include lots of delays for them to catch up to eachother, so they don't trip over commands and we don't
end up with a corrupted image on the display.
-Nolan Adams
